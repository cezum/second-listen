...
function actionRow(event, eid, fresh, calloutText = '') {
  const row = el('article', 'entry action' + (fresh ? ' flash' : ''))
  row.append(el('span', 'eid', eid))
  const main = el('div')
  main.append(el('p', 'task', event.task))
  if (calloutText) main.append(el('span', 'sample-callout', calloutText))
  main.append(el('p', 'task-meta', 'Owner ' + event.owner + ' · due ' + event.deadline + ' · agreed at ' + stamp(event.at_seconds)))
  row.append(main)
  return row
}
