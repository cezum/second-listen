...
Normally the page sends `agent_id` and the session service loads the agent from
the account. With `MODE=inline` the server hands the page the whole
`agents/<name>.jsonc` config instead and the page sends that on
`session.update`, so nothing has to resolve an id. Useful when the stored agent
cannot be resolved by the session service even though REST still returns it.
In this mode the config's `tools` travel with the session — `app.js` sends them,
and the service echoes them back in `session.updated`.

### Check a session before you speak

```sh
AGENT=second-listen python probe_session.py     # both modes
AGENT=second-listen python probe_session.py inline
```

Opens the realtime websocket, sends one `session.update`, and prints
`session.ready` or the error. No microphone and no audio, so it costs nothing
to run before a live call.
